...
char *get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, &line, 0) < 0) //derniere cond pour si fichier  pas lisible , utile pour tester si le ficher et lisible ou non  sans avancer la lecture
		return(NULL);
	readed = 1;
	line = 0;
